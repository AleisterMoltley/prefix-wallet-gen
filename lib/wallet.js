...
function deriveKeypair(mnemonic, scheme = 'standard', account = 0) {
  ...
  if (scheme === 'direct') {
    ...
    ({ key: seed32 } = derivePath(path, seed.toString('hex')));
  }
...
function generateWallet(options = {}) {
  ...
  while (attempts < maxAttempts) {
    ...
    const wallet = deriveKeypair(mnemonic, scheme, account);
